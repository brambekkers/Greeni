...
<template>
  <Card class="!absolute !left-0 !top-0 !translate-x-[-100%] !translate-y-[-40%] !bg-white !text-[#005e5d] !shadow-xl !w-[25rem] !p-3 !font-mono">
    <template #content>
      ...
      <Button v-if="showButtons" label="Show me my results" class="!w-full mb-2 !bg-[#005e5d] !border-[#005e5d] !text-white" @click="$emit('open')" />
      <Button v-if="showButtons" label="Leave me alone" class="!w-full !bg-[#edf7f7] !text-[#005e5d]" @click="status = 'leave'" />
    </template>
  </Card>
</template>
